...
#[test]
fn test_zero() {
    let p_empty_u8: Polynomial<u8, 0> = Polynomial::zero();
    assert!(p_empty_u8.is_empty(), "p_empty_u8 is not empty");
    assert!(!p_empty_u8.is_zero(), "p_empty_u8 is zero");

    let p_zero_i32 = Polynomial::new([0i32]);
    assert!(p_zero_i32.is_zero(), "p_non_zero_f32 is zero");

    let p_zero_f64: Polynomial<f64, 3> = Polynomial::zero();
    assert_eq!(p_zero_f64.coefficients, [0.0f64, 0.0f64, 0.0f64]);
    assert!(p_zero_f64.is_zero(), "p_zero_f64 is not zero");

    let p_non_zero_f32 = Polynomial::new([0.0f32, 0.0f32, 1.0f32]);
    assert!(!p_non_zero_f32.is_zero(), "p_non_zero_f32 is zero");
}
...
fn test_one() {
    ...
    let p_not_one_f64 = Polynomial::new([0.0f64, 2.0f64, 1.0f64]);
    assert!(!p_not_one_f64.is_one(), "p_not_one_f64 is one");

    let p_empty_usize: Polynomial<usize, 0> = Polynomial::one();
    assert!(p_empty_usize.is_empty(), "p_empty_usize is not empty");
    assert!(!p_empty_usize.is_one(), "p_empty_usize is one");
}
